...
void * best_fit_allocator(unsigned int size) {
	...
	int diff = INT_MAX, size2 = (int)size,found = 0;
	struct mem_region *current_region = free_regions, *best_region = NULL;
	while (current_region != NULL) {
		if ((int)current_region->size - size2 < diff){
			diff = (int)current_region->size - size2;
		...
		tmp->pointer = best_region->pointer;
		tmp->size = size;
		...
		if (best_region->size == size) {
			if (best_region == free_regions) {
				free_regions = free_regions->next;
				if (free_regions != NULL) {
					free_regions->prev = NULL;
				}
			}else{
				if (best_region->prev != NULL) {
					best_region->prev->next = best_region->next;
				}
				if (best_region->next != NULL) {
					best_region->next->prev = best_region->prev;
				}
			}
			free(best_region);
		}else{
			best_region->pointer += size;
	...
	return (void*) best_region; // remember to remove this line 
}
